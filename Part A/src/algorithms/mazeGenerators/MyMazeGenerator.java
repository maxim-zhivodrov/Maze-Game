...
public class MyMazeGenerator extends AMazeGenerator
{
    ...
    public Maze generate(int rows, int columns)
    {
        ...
        else if(rows==3 && columns ==3) myMaze.setStartPosition(0,0);
     ...
     * @return
     */
    public ArrayList<Position> Neighbors(Maze maze,Position pos)
    {
        ArrayList <Position> list=new ArrayList<Position>();
    ...
    /**
     * The function checks if the GoalState of the maze is at a corner
     * and if if is, the function breaks one of the walls next to it in order to grant excess to it
     * @param maze
     */
    protected void BreakWall(Maze maze)
    {
        Position tempPos=maze.getGoalPosition();
        ArrayList<Position> list=Neighbors(maze,tempPos);
        int counter=0;
        for(int i=0;i<4;i++)
            if(list.get(i)==null)
                counter++;
    ...
    {
        int x1=(int)(Math.random() * (maze.getRows()-1));
        int y1=(int)(Math.random() * (maze.getColumns()-1));
        Position bestPos=new Position(x1,y1);
        while(!(maze.getCell(bestPos)==0&& !bestPos.equals(maze.getStartPosition())))
        {
            x1=(int)(Math.random() * (maze.getRows()-1));
            y1=(int)(Math.random() * (maze.getColumns()-1));
