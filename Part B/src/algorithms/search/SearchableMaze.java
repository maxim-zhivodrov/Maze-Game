...
public class SearchableMaze implements ISearchable, Serializable
{
    ...
    public ArrayList<AState> getAllPossibleStates(AState currentState)
    {
        ...
        if(pos.getColumnIndex()+1< maze.getColumns()) right=exploredArr[pos.getRowIndex()][pos.getColumnIndex()+1].getPosition();
        if(pos.getColumnIndex()-1 > -1) left=exploredArr[pos.getRowIndex()][pos.getColumnIndex()-1].getPosition();
        if(pos.getRowIndex()-1 > -1) up=exploredArr[pos.getRowIndex()-1][pos.getColumnIndex()].getPosition();
        if(pos.getRowIndex()+1 < maze.getRows()) down=exploredArr[pos.getRowIndex()+1][pos.getColumnIndex()].getPosition();
        ...
        if(states.contains(this.goalState)) //initialize Array if finished
        {
    ...
    private ArrayList<AState> getHypotenuses(Position right,Position left,Position up,Position down)
    {
        ...
        if((right!=null&&up!=null)&&((maze.getCell(up)==0) || (maze.getCell(right)==0))&& maze.getCell(exploredArr[right.getRowIndex()-1][up.getColumnIndex()+1].getPosition())==0)
        {
            ur=exploredArr[right.getRowIndex()-1][up.getColumnIndex()+1];
            hypStates.add(ur);
        }
        else hypStates.add(null);
        if ((right!=null&&down!=null)&&((maze.getCell(down)==0) || (maze.getCell(right)==0))&& maze.getCell(exploredArr[right.getRowIndex()+1][down.getColumnIndex()+1].getPosition())==0)
        {
            dr=exploredArr[right.getRowIndex()+1][down.getColumnIndex()+1];
            hypStates.add(dr);
        }
        else hypStates.add(null);
        if((down!=null&&left!=null)&&((maze.getCell(down)==0) || (maze.getCell(left)==0))&& maze.getCell(exploredArr[left.getRowIndex()+1][down.getColumnIndex()-1].getPosition())==0)
        {
        ...
        if((up!=null&&left!=null)&&((maze.getCell(up)==0) || (maze.getCell(left)==0))&& maze.getCell(exploredArr[left.getRowIndex()-1][up.getColumnIndex()-1].getPosition())==0)
        {
